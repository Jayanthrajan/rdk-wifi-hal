                             ...
                             unsigned int num_report)
{
    ...
    for (itr = 0; itr < num_report; itr++) {
        ...
        rep.rsni = bss->ap_SignalStrength - bss->ap_Noise; /* 255 indicates that RSNI is not available */
        rep.antenna_id = 0; /* unknown */
        rep.parent_tsf = 0x00;
        mac_address_t mac_addr;
        to_mac_bytes(bss->ap_BSSID, mac_addr);
        wifi_hal_dbg_print("%s:%d: JRA %02x..%02x ap_index %d\n", __func__, __LINE__, mac_addr[0], mac_addr[5], ap_index);
        os_memcpy(rep.bssid, mac_addr, ETH_ALEN);

        uint8_t *buf, *pos, *pos1;
        buf = malloc(sizeof(struct rrm_measurement_beacon_report) +  15  + ssid_len + REPORTED_FRAME_BODY_SUBELEM_LEN);
        memset(buf, 0, sizeof(struct rrm_measurement_beacon_report) +  15 + REPORTED_FRAME_BODY_SUBELEM_LEN);
        ...
        *pos++ = ssid_len;
        ...
        size_t data_len = ((pos - pos1) + sizeof(struct rrm_measurement_beacon_report) + REPORTED_FRAME_BODY_SUBELEM_LEN);
    ...
    wifi_hal_send_mgmt_frame(ap_index, backhaul->bssid, wpabuf_head(report), wpabuf_len(report),
    0);
    os_free(wpa_buf);
    return 0;
}

static int wifi_hal_parse_rrm_beacon_req(wifi_interface_info_t *interface, const struct rrm_measurement_beacon_request *req,
    size_t len, wifi_hal_rrm_request_t *meas_req)
{
    wifi_hal_dbg_print("%s:%d ===\n", __func__, __LINE__);
    int op_class;

    if (len < sizeof(*req)) {
        wifi_hal_dbg_print("%s:%d RRM: request element too short, len: %d\n", __func__, __LINE__,
            len);
        return -1;
    }
    ...
    if (req->mode != BEACON_REPORT_MODE_PASSIVE && req->mode != BEACON_REPORT_MODE_ACTIVE) {
        wifi_hal_dbg_print("%s:%d RRM: request mode %u not supported\n", __func__, __LINE__,
            req->mode);
        return -1;
    }
/*
    ...
    */
    ...
    wifi_hal_dbg_print("%s:%d JRA duration:%d mandatory:%d oper_class:%d channel:%d mode:%d %02x...%02x\n", __func__, __LINE__, req->duration, 
                        duration_mandatory, req->oper_class, req->channel, req->mode, req->bssid[0], req->bssid[5]);
    if (meas_req->duration == 0) {
        wifi_hal_dbg_print("%s:%d RRM: duration 0\n", __func__, __LINE__);
        return -1;
    }

    if (meas_req->duration_mandatory) {
        wifi_hal_dbg_print("%s:%d RRM: mandaory duration not supported\n", __func__, __LINE__);
        return -1;
    }
...
static void handle_beacon_rep_response(int ap_index,
        u8 token, const u8 *pos, size_t len, wifi_BeaconReport_t* rep)
{
    // const u8 *end;
    const u8 *attr;
    u8 measurement_rep_mode = 0;
    ...
    wifi_hal_error_print("%s:%d: JRA bssid is %s\n", __func__, __LINE__,to_mac_str(rep->bssid, key));
    ...
    wifi_hal_rrm_report_t *resp)
{
    ...
    /* Pass the reports array to the callback function */
    wifi_hal_error_print("%s:%d: JRA array size is %d\n", __func__, __LINE__, reps.size);
    ...
    wifi_hal_rrm_request_t *req)
{
    ...
    struct ieee80211_mgmt *mgmt = (struct ieee80211_mgmt *)buff;
    //struct rrm_measurement_beacon_report *resp  = (struct rrm_measurement_beacon_report *)malloc(sizeof(struct rrm_measurement_beacon_report ));
    wifi_interface_info_t *interface = get_interface_by_vap_index(apIndex);
    if (interface == NULL) {
        wifi_hal_error_print("%s:%d: WiFi interface not found for vap:%d\n", __func__, __LINE__, apIndex);
        return RETURN_ERR;
    ...
    if (mgmt == NULL || req == NULL) {
        wifi_hal_dbg_print("%s:%d null argument\n", __func__, __LINE__);
        return -1;
    }

    if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.action.u.rrm)) {
        wifi_hal_dbg_print("%s:%d too short measurement request, len: %d\n", __func__, __LINE__,
            len);
        return -1;
    }
    ...
    wifi_hal_dbg_print("%s:%d JRA category code %d\n", __func__, __LINE__, payload[0]);
    payload++; //going to action.
    wifi_hal_dbg_print("%s:%d JRA action code is %d\n", __func__, __LINE__, payload[0]);
    payload++; //going to dialog token.
    wifi_hal_dbg_print("%s:%d JRA dialog token is %d\n", __func__, __LINE__, payload[0]);
    payload++;
    payload++;
    payload++;
    wifi_hal_dbg_print("%s:%d JRA tag number is %d\n", __func__, __LINE__, payload[0]);
    ...
    if (mgmt->u.action.category != WLAN_ACTION_RADIO_MEASUREMENT ||
        ...
        wifi_hal_dbg_print("%s:%d wrong measurement request category/action\n", __func__, __LINE__);
        return -1;
    }
    ...
    if (meas_req->eid != WLAN_EID_MEASURE_REQUEST) {
        wifi_hal_dbg_print("%s:%d wrong measurement request EID %u\n", __func__, __LINE__,
            meas_req->eid);
        return -1;
    }

    if (meas_req->len < 3) {
        wifi_hal_dbg_print("%s:%d measurement request element too short, len: %d\n", __func__,
            __LINE__, meas_req->len);
        return -1;
    }

    if (meas_req->len > len - 2) {
        wifi_hal_dbg_print("%s:%d measurement request element too long, len: %d\n", __func__,
            __LINE__, meas_req->len);
        return -1;
    }

    if (meas_req->mode & MEASUREMENT_REQUEST_MODE_ENABLE) {
        wifi_hal_dbg_print("%s:%d enable bit is not supported\n", __func__, __LINE__);
        return -1;
    }

    if ((meas_req->mode & MEASUREMENT_REQUEST_MODE_PARALLEL) &&
        meas_req->type > MEASURE_TYPE_RPI_HIST) {
        wifi_hal_dbg_print("%s:%d parallel measurements are not supported\n", __func__, __LINE__);
        return -1;
    }

    if (meas_req->mode & MEASUREMENT_REQUEST_MODE_DURATION_MANDATORY) {
        wifi_hal_dbg_print("%s:%d mandatory duration is not supported\n", __func__, __LINE__);
        return -1;
    }
    ...
    default:
        wifi_hal_dbg_print("%s:%d unsupported radio measurement type %u\n", __func__, __LINE__,
            meas_req->type);
        return -1;
    }
