...
int	handle_public_action_frame	(INT ap_index, mac_address_t sta_mac, wifi_publicActionFrameHdr_t *ppublic_hdr, UINT len)
{
    ...
    mgmt = (struct ieee80211_mgmt *)ppublic_hdr;
    action = mgmt->u.action.u.public_action.action;
   
	switch (action) { 

		case wifi_public_action_type_vendor:
			handle_vendor_public_action_frame(ap_index, sta_mac, (unsigned char *)mgmt->u.action.u.public_action.variable, len - IEEE80211_HDRLEN - 2);
			break;

		case wifi_public_action_type_gas_init_req:
			handle_gas_init_public_action_frame(ap_index, sta_mac, (unsigned char *)mgmt->u.action.u.public_action.variable, len - IEEE80211_HDRLEN - 2);
			break;
